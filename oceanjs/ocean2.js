(function() {
  ...
  function loadAll(path, callback) {//is-narrsssisistic.js
    // if (path in modFuncCache) {//解决循环依赖的加载问题
    //   callback()
    // }
    readFile(path, (moduleFunction) => {
      modFuncCache[path] = moduleFunction

      var deps = getDeps(moduleFunction.toString())//['digit-width.js', 'power.js']
